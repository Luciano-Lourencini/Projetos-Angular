...
export class JogoDaVelhaService {
  ...
  private readonly TAM_TAB: number = 3;  //3x3
  private readonly X: number = 1;
  private readonly O: number = 2;
  private readonly VAZIO: number = 0;

  private tabuleiro: any;  //"any" é array
  private numMovimentos: number;
  private vitoria: any;

  private _jogador: number;
  private _showInicio: boolean;   //esses booleanos são flags
  ...
  inicializar(): void {
    ...
    this._jogador = this.X;   //jogador sempre vai ser o X
    this.vitoria = false;
   ...
   * @return void
   */
  inicializarTabuleiro(): void {    //serve para deixar todas as casas vazias
   ...
   * @return boolean
   */
  get showInicio(): boolean {   //encapsulamento de atributo privado  //literalmente o GET
  ...
  jogar(posX: number, posY: number): void {
    // jogada inválida
    if (this.tabuleiro[posX][posY] !== this.VAZIO ||    //se já tem ou se já ganhou
      this.vitoria) {
      return;
    }
    ...
    this.numMovimentos++;
    this.vitoria = this.fimJogo(posX, posY,   //verifica se o jogo acabou
      this.tabuleiro, this._jogador);
    this._jogador = (this._jogador === this.X) ? this.O : this.X;   //muda o jogador   //se foi o "X", agora é o "O"
  ...
  fimJogo(linha: number, coluna: number, 
    ...
    let fim: any = false;    //vai guardar a posição da vitória

    // valida a linha
    if (tabuleiro[linha][0] === jogador && 
      tabuleiro[linha][1] === jogador && 
      tabuleiro[linha][2] === jogador) {
      fim = [[linha, 0], [linha, 1], [linha, 2]];   //posição da vitória
    }
   ...
   * @return void
   */
  cpuJogar(): void {   //simula as jogadas do PC
    // verifica jogada de vitória
    let jogada: number[] = this.obterJogada(this.O);  // verifica se pode vencer
   ...
   * @return nomber[]
   */
  obterJogada(jogador: number): number[] {    //obtém uma jogada válida para a vitória de um jogador
   ...
   * @return boolean
   */
  exibirX(posX: number, posY: number): boolean {    //Retorna se a peça X deve ser exibida naquela coordenada
    return this.tabuleiro[posX][posY] === this.X;  //verifica se nessa posição existe um X
  }
   ...
   * @return boolean
   */
  exibirVitoria(posX: number, posY: number): boolean {   //verifica se tem que marcar com um fundo verde a posição de vitória
   ...
   * @return void
   */
  novoJogo(): void {    //quando clica em "Novo Jogo" chama esse daqui
    this.inicializar();
    this._showFinal = false;
    this._showInicio = false;
    this._showTabuleiro = true;
